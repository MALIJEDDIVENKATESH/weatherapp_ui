...
var container = document.getElementByClassName("dataTable-container");
...
function resetdata() {
  ...
  rows.remove(datatable.activeRows);
}

async function getdata(city) {
  const data = await fetch(
    `https://api.opencagedata.com/geocode/v1/json?q=${city}&key=4b76290ce624452f949be23fa36d6262&pretty=1`
  );
...
async function fetchdata() {
  ...
  try {
    const data = await fetch(
      `https://api.openweathermap.org/data/2.5/weather?q=${input}&units=metric&appid=374abe7635cad005595d3765035c9d63`
    );
  ...
  try {
    ...
    const places = await fetch(
      `https://api.openweathermap.org/data/2.5/find?&lat=${latitude}&lon=${longitude}&cnt=25&units=metric&appid=374abe7635cad005595d3765035c9d63`
    );
